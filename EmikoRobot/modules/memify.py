...
async def handler(event):
    ...
    if not event.reply_to_msg_id:
        await event.reply("`Pʀᴏᴠɪᴅᴇ sᴏᴍᴇ ᴛᴇxᴛ ᴛᴏ ᴅʀᴀᴡ! Aɴᴅ ʀᴇᴘʟʏ ᴛᴏ ɪᴍᴀɢᴇ/Sᴛɪᴄᴋᴇʀs EXAMPLE: /mmf ᴛᴇxᴛ`")
        return
    ...
    msg = await event.reply("```Mᴇᴍɪғʏɪɴғ ᴛʜɪs ɪᴍᴀɢᴇ! (」ﾟﾛﾟ)｣ ```")
    text = str(event.pattern_match.group(1)).strip()
    if len(text) < 1:
        return await msg.edit("Yᴏᴜ ᴍɪɢʜᴛ ᴡᴀɴᴛ ᴛᴏ ᴛʀʏ `/mmf ᴛᴇxᴛ`")
