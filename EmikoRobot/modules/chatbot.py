...
import html
import requests
import EmikoRobot.modules.sql.kuki_sql as sql
...
def kukiadd(update: Update, context: CallbackContext) -> str:
    ...
    if match:
        ...
        if is_kuki:
            ...
            update.effective_message.edit_text(
                "Cʜᴀᴛʙᴏᴛ ᴇɴᴀʙʟᴇᴅ ʙʏ {} .".format(mention_html(user.id, user.first_name)),
                parse_mode=ParseMode.HTML,
            )
...
def kuki(update: Update, context: CallbackContext):
    ...
    msg = f"Cʜᴏᴏsᴇ ᴀɴ ᴏᴘᴛɪᴏɴ"
    keyboard = InlineKeyboardMarkup([[
        InlineKeyboardButton(
            text="Eɴᴀʙʟᴇ ᴛᴇᴅᴅʏ ᴀɪ",
            callback_data="add_chat({})")],
       [
        InlineKeyboardButton(
            text="Dɪsᴀʙʟᴇ ᴛᴇᴅᴅʏ ᴀɪ",
...
def list_all_chats(update: Update, context: CallbackContext):
    ...
    text = "<b>𝗧𝗲𝗱𝗱𝘆 𝗰𝗵𝗮𝘁𝗯𝗼𝘁 𝗔𝗜 𝗲𝗻𝗮𝗯𝗹𝗲𝗱 𝗰𝗵𝗮𝘁𝘀 👇</b>\n"
    for chat in chats:
        try:
...
__help__ = """
Yᴏᴜ ᴄᴀɴ ᴛᴀʟᴋ ᴛᴏ ᴛʜᴇ ᴛᴇᴅᴅʏ ʙᴏᴛ ᴠɪᴀ ᴛʜɪs ᴍᴏᴅᴜʟᴇ. . . 
*Aᴅᴍɪɴs ᴏɴʟʏ ᴄᴏᴍᴍᴀɴᴅ*:
  ➢ `/Chatbot`*:* Sʜᴏᴡs ᴄʜᴀᴛʙᴏᴛ ᴄᴏɴᴛʀᴏʟ ᴘᴀɴᴇʟ
  𝗔𝗣𝗜 𝗖𝗥𝗘𝗗𝗜𝗧𝗦 :-> *@smokerr_xd*
"""
